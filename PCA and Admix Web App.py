...
def plot():
    ...
    # Alias for SAMPLE_POP to join with PCA
    sample_pop_alias = aliased(SAMPLE_POP)

    # Filter PCA data based on selected populations
    for population in selected_populations:
        pca_data = (
            PCA.query
            .join(sample_pop_alias, PCA.SAMPLE_ID == sample_pop_alias.SAMPLE_ID)
            .filter(sample_pop_alias.POPULATION == population)
            .all()
        )
    ...
    for superpopulation in selected_superpopulations:
        ...
        pca_data = (
            PCA.query
            .join(sample_pop_alias, PCA.SAMPLE_ID == sample_pop_alias.SAMPLE_ID)
            .filter(func.substr(sample_pop_alias.SUPERPOPULATION, 1, 3) == superpopulation_code)
            .all()
        )
        pc1_values = [entry.PC1 for entry in pca_data]
        pc2_values = [entry.PC2 for entry in pca_data]
...
def SNPtable():
    if request.method == 'POST':
        ...
        # result = db.session.query(VARIANTS, AlleleFrequency). \
        #     filter(VARIANTS.SNP_ID == AlleleFrequency.SNP_ID). \
        #     filter(filter_condition). \
        #     add_columns(VARIANTS.POS, VARIANTS.REF, VARIANTS.ALT, VARIANTS.GENE, VARIANTS.CLINICAL_RELEVANCE, * [getattr(AlleleFrequency, population) for population in selected_populations]). \
        #     limit(150).all()
